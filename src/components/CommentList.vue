      ...
      <div class="parent-reply reply">
        <div class="user-info">
          <xr-avatar
            ...
            :id="item.userInfo.user_id || item.userInfo.id"
            :size="34"
            :src="item.userInfo.thumb_img||item.userInfo.img"
            :disabled="false"
            class="user-img" />
          <div class="user-info__bd">
            <div class="user">{{ item.userInfo.realname }}</div>
            <div class="time">
              {{ item.create_time }}
            </div>
          </div>
      ...
      <template v-if="item.childCommentList && item.childCommentList.length > 0">
        <div
          ...
          <div class="child-reply reply">
            <div class="user-info">
              <xr-avatar
                :name="child.userInfo.realname"
                :id="child.userInfo.user_id"
                :size="34"
                :src="child.userInfo.img"
                :disabled="false"
                class="user-img" />
              <div class="user-info__bd">
                <div class="user">
                  {{ child.userInfo.realname }}
                </div>
                <div class="time">
                  {{ child.create_time }}
                </div>
...
export default {
  ...
  methods: {
    ...
    handleReply(data) {
      ...
      if (f_comment) {
        params.main_id = f_comment.comment_id
        params.comment_id = f_comment.childCommentList.comment_id || f_comment.comment_id
      }
      if (c_comment) {
        params.comment_id = c_comment.comment_id
      }
      ...
      setCommentAPI(params).then(res => {
        ...
        if (c_comment) {
          res.data.replyuserInfo = c_comment.userInfo
        } else {
          res.data.replyuserInfo = f_comment.userInfo
        }
