  ...
  <div class="main">
    ...
    <div
      ...
      <log-cell
        ...
        <div
          class="relate-cell"
          @click="checkRelationDetail(item.activity_type, item.activity_type_id, true)">
          <i
            :class="item.activity_type | crmIconClass"
            class="relate-cell-icon" />
          <span class="relate-cell-type">{{ item.activity_type | crmName }}-</span>
          <span class="relate-cell-name">{{ item.activity_type_name }}</span>
        </div>
      </log-cell>
      <p
        v-if="loading"
...
export default {
  ...
  methods: {
      ...
      crmIndexGetRecordListAPI(params)
        .then(res => {
          ...
          if (this.filterForm.subUser != params.subUser) {
            ...
            this.count = res.data.dataCount || 0
     ...
     */
    userChange(data) {
      this.filterForm.user_id = data.value.length > 0 ? data.value[0].id : ''
    },
