<template>
  <div class="main">
    ...
    <examine-create-view
      v-if="isCreate"
      :category-id="createInfo.category_id"
      :type="createInfo.type"
      :category-title="createInfo.category_name"
      :action="createAction"
...
export default {
  ...
  mounted() {
    ...
    this.getSelectList()
    this.tabsSelectValue = this.examineType == 'my' ? '0' : '0'
  },
  ...
  methods: {
    ...
    getList() {
      ...
      let request = null
      const status = this.tabsSelectValue == 'all' ? 'all' : this.tabsSelectValue
      if (this.examineType == 'my') {
        params.check_status = status
        request = oaExamineMyCreateIndexAPI
      } else if (this.examineType == 'wait') {
        // params.check_status = status
        params.examine_by = status == '0' ? 1 : status == 'all' ? 'all' : 2
      ...
      request(params)
        .then(res => {
          ...
          const status = this.tabsSelectValue == 'all' ? 'all' : this.tabsSelectValue
          let pass = false
          if (this.examineType == 'my' && params.check_status == status) {
            pass = true
          } else if (this.examineType == 'wait' && params.examine_by == (status == '0' ? 1 : status == 'all' ? 'all' : 2)) {
            pass = true
          }
      ...
      oaAllExamineCategoryListAPI()
        .then(res => {
          this.selectList = res.data.map(item => {
            ...
            iconItem.command = item.category_id
            return iconItem
          })
    ...
    cellHandle(type, data, index) {
      ...
      if (type == 'detail') {
        ...
        this.createAction = { type: 'update', id: data.examine_id, data: data }
        this.isCreate = true
      } else if (type == 'delete') {
