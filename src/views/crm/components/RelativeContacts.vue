      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <flexbox justify="center">
            <span
              v-if="contactsId == scope.row.contacts_id"
              class="chief">
  ...
  methods: {
    ...
    checkRelativeInfos(data) {
      if (data.data.length > 0) {
        const params = { business_id: this.id, is_relation: 1 }
        params.contacts_id = data.data
          .map(item => {
    ...
    unRelevanceHandleClick() {
      if (this.selectionList.length == 0) {
        ...
        this.$confirm('确认取消关联?', '提示', {
          ...
          .then(() => {
            const params = { business_id: this.id, is_relation: 0 }
            params.contacts_id = this.selectionList
              .map(item => {
                return item.contacts_id
              })
    ...
    setChieflyContacts(data) {
      ...
      const params = { contacts_id: data.row.contacts_id }
      params[this.crmType + '_id'] = this.id
      this.loading = true
      request(params)
        .then(res => {
          this.$message.success('操作成功')
          this.$emit('update:contactsId', data.row.contacts_id)
          this.loading = false
        })
        .catch(data => {
          this.loading = false
